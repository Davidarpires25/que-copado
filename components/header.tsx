...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-[#F0EBE1] bg-[#FBF5E6] shadow-sm">
          ...
          {navLinks.map((link) => (
            link.action ? (
              <Button
                key={link.href}
                variant="ghost"
                className="text-[#2D1A0E] hover:text-[#2D1A0E] hover:bg-[#F0EBE1] font-semibold"
                onClick={link.action}
              >
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="md:hidden border-t border-[#F0EBE1] bg-[#FBF5E6] overflow-hidden"
          >
              ...
              {navLinks.map((link) => (
                link.action ? (
                  <motion.div
                    key={link.href}
                    whileTap={{ scale: 0.98 }}
                    className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#F0EBE1] transition-colors cursor-pointer"
                    onClick={() => {
                      link.action()
                      setIsMenuOpen(false)
                    }}
                  >
                    ...
                    <span className="font-semibold text-[#2D1A0E]">{link.label}</span>
                  </motion.div>
                ) : (
                  <Link
                    key={link.href}
                  ...
                  >
                    <motion.div
                      whileTap={{ scale: 0.98 }}
                      className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#F0EBE1] transition-colors"
                    >
                      <div className="w-10 h-10 rounded-full bg-[#FEC501] flex items-center justify-center">
                        <link.icon className="h-5 w-5 text-black" />
                      </div>
