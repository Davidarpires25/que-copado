...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the storefront root (/) to start the checkout flow and test pickup behavior.
        await page.goto("http://localhost:3000/", wait_until="commit", timeout=10000)
        
        # -> Click the add-to-cart button for a product (Classic) to add an item to the cart.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/section[2]/div/div/div[2]/div/div/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Navigate to the storefront checkout page (/checkout) to continue the pickup verification flow.
        await page.goto("http://localhost:3000/checkout", wait_until="commit", timeout=10000)
        
        # -> Click the 'Retiro en local' (Pickup) delivery option to switch to pickup mode.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div[2]/div/div/div[2]/div/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
