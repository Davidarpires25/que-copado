...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields and click 'Iniciar Sesion' to sign in to the admin dashboard.
        ...
        # -> Open the 'Categorías' section in the admin sidebar to manage categories.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/aside/nav/div[3]/div[2]/a[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Abrir el formulario para crear una nueva categoría haciendo clic en 'Nueva Categoría' y luego completar nombre='TC001' y un orden válido, guardar y verificar que aparezca en la lista.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[2]/main/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the category name with 'TC001', scroll to reveal the sort order field, then fill the sort order and save the category.
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=TC001').first).to_be_visible(timeout=3000)
