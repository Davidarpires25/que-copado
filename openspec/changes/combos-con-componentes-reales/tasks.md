...
- [ ] 1.1 Migración: tabla `product_components` (padre, hijo, cantidad) con sus
      ...
      sus policies.
- [ ] 1.2 Migración: fila `combo` en `product_types`, con `sends_to_kitchen` y
...
- [ ] 1.3 Tipos de TypeScript y el helper que dice si un producto es combo.
      Verificación: `npx tsc --noEmit` pasa.

## 2. Vender un combo descuenta lo correcto

- [ ] 2.1 En `lib/server/stock-deduction.ts`, expandir los combos a sus
      ...
      unidad de la bebida, con sus movimientos.
- [ ] 2.2 La cantidad se multiplica: tres combos descuentan tres veces cada
      componente. Verificación: vender 3 y comprobar las cantidades en
      `stock_movements`.
- [ ] 2.3 Cancelar un pedido con combos devuelve todo lo descontado.
      Verificación: cancelar la venta anterior y ver el stock como estaba.
- [ ] 2.4 Un combo sin componentes no se puede vender. Verificación: crear uno
      vacío e intentar cobrarlo.

## 3. Cocina y ticket

- [ ] 3.1 `sendToKitchen` arma la comanda desde los componentes del combo, cada
...
- [ ] 4.1 En el alta y edición de producto, cuando el tipo es `combo`, se cargan
      componentes en vez de recetas: buscar un producto, agregarlo con su
      cantidad, quitarlo. Verificación: armar un combo en local y ver sus
...
- [ ] 4.3 El costo del combo se calcula sumando sus componentes y se muestra al
